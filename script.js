...
const form = document.getElementById('my-form'),
      sub = document.getElementById('button-sub'),
      clr = document.getElementById('button-clr'),
      txt = document.getElementById('field'),
      res = document.getElementById('response_text');
...
function passValue(field_value) {
  res.innerHTML = field_value;
}
...
function callback() {
  sub.classList.remove('animate');
}

sub.addEventListener("webkitAnimationEnd", callback, false);

if (form.attachEvent) {
  form.attachEvent("submit", handleClickOnSubmit);
} else {
  form.addEventListener("submit", handleClickOnSubmit);
}
...
// Path begins at 'start' point and ends on 'end' point.

function Firework(startX, startY, endX, endY) {
  // set current coordiantes.
  this.x  = startX;
  this.y = startY;
  ...
  // While the trail length remains, add current point to trails list.
  while(this.trailLength--) {
    this.trail.push([this.x, this.y]);
  }
  //Calculate the angle to travel from strat to end point.
...
Firework.prototype.update = function(index) {
  ...
  if (this.distanceTraveled >= this.distanceToEnd) {
    ...
    // Create particle explosion at end point. Important not o use this. and this.y, 
...
Firework.prototype.draw = function() {
  // Begin a new path for fierwork trail.
  context.beginPath();
  ...
  if (FIREWORK_TARGET_INDICATOR_ENABLED) {
    // Being a new path for end position animation.
    context.beginPath();
    // Create a pulsing cirecle at the end point with targetRadius.
...
// Clean up the canvas by removing older trails.
// ...
function cleanCanvas() {
  // set 'destinatino' composite mode, so additional fill doesn't remove non-overlappign content.
  context.globalCompositeOperation = 'destination-out';
  // Set alpha level of contetn to remove.
...
function launchAutomatedFirework() {
  ...
  if (ticksSinceFireworkAutomated >= random(TICKS_PER_FIREWORK_AUTOMATED_MIN, TICKS_PER_FIREWORK_AUTOMATED_MAX)) {
    ...
    if (!isMouseDown) {
      ...
      // Set end position to random position, somehwer in the top half of screen.
